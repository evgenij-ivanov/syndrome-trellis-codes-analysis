...
def get_user_message() -> np.ndarray:
    ...
    while True:
        ...
        # txt_input = ''.join(choice(alphabet) for i in range(int(len(cover) * payload)))
...
def img_to_lsb() -> np.ndarray:
    img: Union[Image, np.ndarray] = Image.open(src_img_path).convert('L')
...
def get_h(sub_h, payload_size, message_size):
    ...
    def place_submatrix(h_row, h_column):
        cur_row: int
        for cur_row in range(sub_height):
            for cur_column in range(sub_width):
